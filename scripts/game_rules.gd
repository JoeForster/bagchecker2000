...
class Rule:
	...
	var max_num_of_matching_item : int # TODO we only support 0 or 1

func rules_match(rule_1 : Rule, rule_2 : Rule) -> bool:
	assert(rule_1.restricted_item_tag)
...
func item_really_breaks_rule(item : ScannedShape) -> bool:
	return any_tag_breaks_rule(item.current_item_tags)

func item_shape_potentially_breaks_rule(item : ScannedShape) -> bool:
	return any_tag_breaks_rule(item.shape_possible_item_tags)
...
func _generate_new_rule_tagged():
	...
	if current_rules.size() < max_rule_count:
		...
		for new_rule_tag in all_item_tags:
			...
			new_rule.max_num_of_matching_item = 0

			var rule_matches = func(check_rule : Rule) -> bool:
				return rules_match(new_rule, check_rule)
			
			if current_rules.any(rule_matches):
				print("rejecting existing tag rule: %s/%d" % [new_rule.restricted_item_tag, new_rule.max_num_of_matching_item])
			else:
