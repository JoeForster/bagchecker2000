...
class ShiftResults:
	...
	var num_false_clears = 0
	var num_false_flags = 0
	var time_spent = 0
	...
	func get_total_score(game_rules : GameRules, shift_rules : ShiftRules) -> int:
		...
		total_score -= num_false_clears * game_rules.penalty_per_mistake
		total_score -= num_false_flags * game_rules.penalty_per_mistake
